...
export default function Footer() {
  ...
  // ACCESSIBILITY FIX: Bumped text from slate-200 to slate-100 for higher contrast on mobile
  const linkClass = "block py-3 text-slate-100 hover:text-[#A78BFA] transition-colors";

  return (
    <footer className="bg-slate-950 text-slate-50 font-sans border-t-4 border-[#8B5CF6]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8 lg:gap-8">
          ...
          <div className="space-y-4">
            <Link href="/" prefetch={false} className="inline-block p-1" aria-label="TopTenUAE Home">
              <div className="bg-white p-2 rounded-lg shadow-sm inline-block">
                {/* CLS FIX: Explicit width/height matching the rendered aspect ratio */}
                <Image 
            ...
            <p className="text-sm leading-relaxed font-bold text-slate-50">
              The Best of the UAE, Ranked.
            </p>
            {/* ACCESSIBILITY FIX: Removed opacity from bg to ensure contrast ratio is calculable and valid */}
            <div className="flex items-center gap-2 text-sm font-semibold text-emerald-200 bg-emerald-950 w-fit px-3 py-1.5 rounded-full border border-emerald-800">
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm">
              ...
              <li><Link href="/tech" prefetch={false} className={linkClass}>Tech</Link></li>
              ...
              <li>
                <Link href="/finance-tools" prefetch={false} className="group block py-3 hover:text-white transition-colors">
                   <div className="flex items-center gap-2">
                     ...
                     {/* ACCESSIBILITY FIX: 
                        Changed bg from 'violet-800' to '#A78BFA' (Light Purple) and text to 'slate-950' (Dark).
                        This creates a High Contrast "Light on Dark" badge that passes WCAG AAA.
                     */}
                     <span className="bg-[#A78BFA] text-slate-950 text-[12px] font-bold px-1.5 py-0.5 rounded animate-pulse">NEW</span>
                   </div>
                </Link>
              </li>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-6">
              <Link 
                ...
                className="h-12 w-12 flex items-center justify-center bg-slate-800 text-slate-100 rounded-full hover:bg-[#1877F2] hover:text-white transition-all"
              >
              ...
              <Link 
                ...
                className="h-12 w-12 flex items-center justify-center bg-slate-800 text-slate-100 rounded-full hover:bg-[#1DA1F2] hover:text-white transition-all"
              >
              ...
              <Link 
                ...
                className="h-12 w-12 flex items-center justify-center bg-slate-800 text-slate-100 rounded-full hover:bg-[#E4405F] hover:text-white transition-all"
              >
                <Instagram className="w-5 h-5" />
              </Link>
            </div>
            
            <Link href="/contact-us" prefetch={false} className="inline-flex items-center gap-2 text-sm text-slate-100 hover:text-white transition-colors border border-slate-700 rounded-lg px-4 py-3 hover:bg-slate-800">
      ...
      <div className="bg-slate-950 border-t border-slate-800 py-8">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="mb-6 pb-6 border-b border-slate-950">
            {/* ACCESSIBILITY FIX: Bumped text-slate-300 -> text-slate-200 for better readability on dark bg */}
            <p className="text-[12px] text-slate-200 leading-relaxed text-justify md:text-left">
              <strong className="text-white">Affiliate Disclosure:</strong> TopTenUAE is a participant in the Amazon Services LLC Associates Program...
            </p>
          </div>
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-slate-200">
