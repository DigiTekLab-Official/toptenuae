...
const QUERY = `*[slug.current == $slug][0]{
  ...
  "linkedProduct": product->{ 
      title, brand, mainImage{asset, alt}, affiliateLink, retailer 
  },
  ...
  // Image Logic (Smart Coalesce for Deals)
  ...
  listItems[] { 
    ...
    product->{ 
      title, "slug": slug.current, mainImage { asset, alt, "url": asset->url }, 
      affiliateLink, retailer, priceTier, price, currency, availability, 
      realComplaint, customerRating, reviewCount, verdict, keyFeatures, pros, cons, itemDescription 
    } 
  }
}`;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const metaTitle = data.seo?.metaTitle || `${data.title} - TopTenUAE`;
  const metaDesc = data.seo?.metaDescription || data.description;
  
  // Smart Image Selection (Deal > Product > Default)
  const ogImage = data.imageUrl || data.linkedProduct?.mainImage?.asset?.url || 'https://toptenuae.com/og-default.jpg';
  ...
  return {
    ...
    alternates: { 
      canonical: data.seo?.canonicalUrl || `https://toptenuae.com/${category}/${slug}` 
    },
    openGraph: {
      title: metaTitle,
      description: metaDesc,
      type: data._type === 'tool' ? 'website' : 'article',
...
export default async function Page({ params }: PageProps) {
  ...
  // 2. Deals must be in /deals/
  if (data._type === 'deal' && category !== 'deals') {
     permanentRedirect(`/deals/${slug}`);
  }
  ...
  // ---------------------------------------------------------------------------
  // 1. RENDER STRATEGY: TOOL (Optimized for Calculators)
  // ---------------------------------------------------------------------------
  if (data._type === "tool") {
    const toolSchema = {
      "@context": "https://schema.org",
      "@type": "SoftwareApplication",
      "name": safeJsonLdString(data.title),
      "description": safeJsonLdString(data.description),
      ...
      <main className="min-h-screen bg-slate-50 font-sans">
        ...
        <section className="bg-[#4b0082] relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 pt-12 pb-24 lg:pt-10 lg:pb-14 relative z-10">
            <div className="mb-4">
              <Breadcrumb
                categoryName={data.category?.menuLabel || data.category?.title || "Tools"}
              ...
              <div className="flex-1 text-center lg:text-left mt-4 text-white">
                ...
                <h1 className="text-4xl lg:text-5xl font-black leading-tight mb-6">
                  ...
                  {data.heroTitleSuffix && (
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 to-amber-500">{data.heroTitleSuffix}</span>
                  )}
                </h1>
                <div className="text-lg text-purple-100 mb-8 leading-relaxed opacity-90 max-w-2xl mx-auto lg:mx-0">
                    ...
                    return (
                      ...
                      >
                        <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl border border-slate-300 hover:border-[#4b0082]/30 transition-all h-full flex flex-col overflow-hidden">
                          <div aria-hidden="true" className="absolute top-0 right-0 bg-[#4b0082]/5 w-24 h-24 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110 group-hover:bg-[#4b0082]/10"></div>
                          <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 transition-colors duration-300 ${config.iconBg}`}>
                            <ToolIcon aria-hidden="true" className={`w-7 h-7 transition-colors duration-300 ${config.iconColor}`} />
                          </div>
  ...
  if (data._type === "product" || data._type === "deal") {
    ...
    // Fallback: If it's a Deal, map the Deal Price to the standard Price field
    // so ProductTemplate can render it without changing the template code.
    ...
    const productSchema = {
      ...
      "offers": {
        "@type": "Offer",
        "url": displayAffiliate,
        "priceCurrency": displayCurrency,
        "price": displayPrice,
        "availability": data.availability || "https://schema.org/InStock",
        "priceValidUntil": data.dealEndDate, // ✅ Exclusive to Deals
      ...
      ...(data.customerRating && {
        "aggregateRating": {
          ...
          "reviewCount": data.reviewCount?.toString().replace(/[^0-9]/g, '') || "1"
        }
      })
    };

    // Construct a merged data object for the template
    const templateData = {
      ...data,
      title: displayTitle,
      price: displayPrice,
    ...
    return (
      <>
        ...
        <ProductTemplate data={templateData} />
      </>
    );
  }
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-5">
        <main className="flex-1 min-w-0 bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
          <header className="border-b border-gray-100 pb-6">
            {data._type === "topTenList" && (
              <span className="inline-block bg-purple-100 text-purple-700 text-sm font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-4">
                Top 10 Review
              </span>
            )}
